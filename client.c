...
// ENTER THEN WE HAVE TO STOP INPUT
// THEN EMPTY STRING WILL GO TO SERVER M

// IF ELSE   where the message is detected the server should check password is   empty
// main server will remeber that it is the guest bool is guest  memeber have to send it to client that its a guest & the client will reposond
// guess can do see availablity request


// if password is username is matched, if 
int main(void)
{
    char authstr[100];
    char *querrystr;
